...
namespace BovineLabs.Core.Editor.Inspectors
{
    ...
    public class DynamicHashMapElement<TBuffer, TKey, TValue> : VisualElement
        where TBuffer : unmanaged, IDynamicHashMap<TKey, TValue>
        where TKey : unmanaged, IEquatable<TKey>
        where TValue : unmanaged
    {
        private readonly DynamicHashMapListElement<TBuffer, TKey, TValue> listElement;
        private readonly DynamicHashMapSearchElement<TBuffer, TKey, TValue>? searchElement;
        ...
        public DynamicHashMapElement(object inspector, List<SearchView.Item>? items = default, TValue defaultValue = default)
        {
            ...
            this.listElement = new DynamicHashMapListElement<TBuffer, TKey, TValue>(inspector, 0);
            this.Add(this.listElement);

            if (hasItems)
            {
                this.searchElement = new DynamicHashMapSearchElement<TBuffer, TKey, TValue>(inspector, items!, defaultValue, 0);
            }
