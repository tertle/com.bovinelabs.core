        ...
        where TC2 : unmanaged, IColumn<T2>
    {
        ...
        internal bool Remove(TKey key)
        {
            ...
            while (entryIdx >= 0 && entryIdx < Capacity)
            {
                if (UnsafeUtility.ReadArrayElement<TKey>(KeyHash.Keys, entryIdx).Equals(key))
                {
                    ...
                    Column1.Remove(entryIdx);
                    Column2.Remove(entryIdx);
        ...
        internal bool TryGetValue(TKey key, out TValue item, out T1 column1, out T2 column2)
        {
            ...
            if (idx != -1)
            {
                ...
                column1 = Column1.GetValue(idx);
                column2 = Column2.GetValue(idx);
                return true;
            }
        ...
        {
            return Column1.GetValue(index);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal T2 GetColumn2AtIndex(int index)
        {
            return Column2.GetValue(index);
        }
        ...
        internal void GetValueAtIndex(int index, out TKey key, out TValue item, out T1 column1, out T2 column2)
        {
            ...
            column1 = GetColumn1AtIndex(index);
            column2 = GetColumn2AtIndex(index);
        }
